import { initialState, appReducer, ActionType } from "./reducer";

describe("Mob reducers", () => {
  it("Adds a member of the mob", () => {
    initialState.mob = ["Claire"];

    const action = { type: ActionType.ADD_MEMBER, payload: "J" };
  ...
  it("Removes a member of the mob", () => {
    ...
    const action = { type: ActionType.REMOVE_MEMBER, payload: 0 };
    const result = appReducer(initialState, action);
  ...
  it("Rotates the members of the mob", () => {
    ...
    const action = { type: ActionType.ROTATE_MEMBERS };
    const result = appReducer(initialState, action);
    expect(result.mob).toEqual(["J", "Claire"]);
  });

  it("Sets the members of the mob", () => {
    const action = { type: ActionType.SET_MEMBERS, payload: ["Claire", "J"] };
    const result = appReducer(initialState, action);
    expect(result.mob).toEqual(["Claire", "J"]);
  });
});
...
describe("Time reducers", () => {
  it("Decrements the time remaining and increments the elapsed time", () => {
    ...
    const action = { type: ActionType.DECREMENT_TIMER };
    const result = appReducer(initialState, action);
  ...
  it("Resets the time remaining", () => {
    ...
    const action = { type: ActionType.RESET_TIMER };
    const result = appReducer(initialState, action);
  ...
  it("Toggles the running state", () => {
    ...
    const action = { type: ActionType.TOGGLE_TIMER };
    const result = appReducer(initialState, action);
  ...
  it("Sets the duration and remaining (if less than remaining", () => {
    ...
    const action = { type: ActionType.SET_DURATION, payload: 200 };
    const result = appReducer(initialState, action);
  ...
  it("It adds two extra minutes", () => {
    ...
    const action = { type: ActionType.ADD_TWO_MINUTES };
    const result = appReducer(initialState, action);
